...
using namespace osd_ts;

OsdTs::OsdTs() : font_(nullptr),
                 run_(false),
                 thread_(nullptr),
                 init_(false)
{
...
int OsdTs::Initialize(const Params &params)
{
    if (init_)
        return KInitialized;
    ...
    int font_w, font_h;
    std::string ts_str = Utils::GetLocalTime(params_.ts_format);
    TTF_SizeUTF8(font_, ts_str.c_str(), &font_w, &font_h);
    ...
    thread_ = std::unique_ptr<std::thread>(new std::thread([this]() {
        ...
        {
            std::string ts_str = Utils::GetLocalTime(params_.ts_format);
            SDL_Surface *sdl_txtsurface = TTF_RenderUTF8_Solid(font_, ts_str.c_str(), forecol);
